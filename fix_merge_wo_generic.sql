...
DECLARE
  ...
  ------------------------------------------------------------------
  c_old_no   constant text := 'WO-GANTI-SDU';   -- WO YANG SALAH (akan dihapus)
  c_new_no   constant text := 'WO-GANTI-BARU';  -- WO YANG BENAR (akan menampung)
...
-- =====================================================================
WITH p AS (
  SELECT 'WO-GANTI-SDU'::text  AS old_no,   -- WO yang dihapus
         'WO-GANTI-BARU'::text AS new_no    -- WO yang dipertahankan
)
...
WHERE v.license_plate = (
        SELECT upper(btrim(v2.license_plate))
         ...
         WHERE wo2.wo_number = 'WO-GANTI-BARU'
       )
...
WHERE wo.wo_number = 'WO-GANTI-BARU'
ORDER BY po.po_number;

-- D) Sisa teks nomor WO sumber (harus 0 semua)
SELECT 'journal_entries' AS kolom, count(*) AS sisa
  FROM journal_entries
 WHERE coalesce(voucher_no,'')   LIKE '%WO-GANTI-SDU%'
    OR coalesce(reference,'')    LIKE '%WO-GANTI-SDU%'
    OR coalesce(description,'')  LIKE '%WO-GANTI-SDU%'
UNION ALL
SELECT 'sales_invoices.invoice_number', count(*)
  FROM sales_invoices WHERE invoice_number = 'INV-WO-GANTI-SDU'
UNION ALL
SELECT 'activity_logs.details', count(*)
  FROM activity_logs WHERE details LIKE '%WO-GANTI-SDU%'
UNION ALL
SELECT 'activity_logs.meta.wo_number', count(*)
  FROM activity_logs WHERE meta ->> 'wo_number' = 'WO-GANTI-SDU';
...
-- WHERE wo.wo_number IN ('WO-GANTI-SDU', 'WO-GANTI-BARU')
-- ORDER BY wo.created_at;
--
...
-- SELECT EXISTS (SELECT 1 FROM sales_invoices WHERE invoice_number = 'INV-WO-GANTI-SDU')  AS punya_inv_sumber,
--        EXISTS (SELECT 1 FROM sales_invoices WHERE invoice_number = 'INV-WO-GANTI-BARU') AS punya_inv_tujuan;
