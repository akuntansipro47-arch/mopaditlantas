...
const WorkOrderV2 = () => {
  ...
  const handleFinishWO = async (wo: WorkOrder) => {
    if (!window.confirm("Apakah Anda yakin ingin menyelesaikan Work Order ini? Ini akan memvalidasi semua sparepart yang dibutuhkan.")) return;

    try {
      const vehicle_entry_id = wo.vehicle_entry_id;
      const woId = wo.id;

      // 1. Get estimated parts for the vehicle entry
      const { data: estimatedParts, error: estimatedError } = await supabase
        .from('vehicle_entry_spareparts')
        .select('goods_id, qty')
        .eq('vehicle_entry_id', vehicle_entry_id);

      if (estimatedError) throw new Error(`Gagal cek estimasi part: ${estimatedError.message}`);

      // If no parts were estimated, allow closing immediately.
      if (!estimatedParts || estimatedParts.length === 0) {
        const { error: updateError } = await supabase.from('work_orders').update({ status: 'COMPLETED' }).eq('id', woId);
        if (updateError) throw updateError;
        toast.success("Work Order berhasil diselesaikan (tidak ada part estimasi).");
        fetchWOs();
        return;
      }

      // 2. Get issued parts for this specific WO
      const { data: goodIssues, error: goodIssuesError } = await supabase
        .from('good_issues')
        .select('id')
        .eq('work_order_id', woId);

      if (goodIssuesError) throw new Error(`Gagal cek pengeluaran barang: ${goodIssuesError.message}`);

      let issuedParts: { goods_id: string; qty: number }[] = [];
      if (goodIssues.length > 0) {
        const goodIssueIds = goodIssues.map(gi => gi.id);
        const { data, error } = await supabase
          .from('good_issue_details')
          .select('goods_id, qty')
          .in('good_issue_id', goodIssueIds);
        if (error) throw new Error(`Gagal cek detail pengeluaran barang: ${error.message}`);
        issuedParts = data || [];
      }

      // 3. Aggregate and compare quantities
      const estimatedMap = estimatedParts.reduce((map, part) => {
        map.set(part.goods_id, (map.get(part.goods_id) || 0) + part.qty);
        return map;
      }, new Map<string, number>());

      const issuedMap = issuedParts.reduce((map, part) => {
        map.set(part.goods_id, (map.get(part.goods_id) || 0) + part.qty);
        return map;
      }, new Map<string, number>());

      let allPartsSufficient = true;
      const missingPartsMessages: string[] = [];

      const allGoodsIds = Array.from(estimatedMap.keys());
      if (allGoodsIds.length > 0) {
          const { data: goodsData, error: goodsError } = await supabase
            .from('goods')
            .select('id, name')
            .in('id', allGoodsIds);
          if (goodsError) throw new Error(`Gagal mengambil nama barang: ${goodsError.message}`);
          const goodsMap = new Map(goodsData?.map(g => [g.id, g.name]));

          for (const [goods_id, estimatedQty] of estimatedMap.entries()) {
            const issuedQty = issuedMap.get(goods_id) || 0;
            if (issuedQty < estimatedQty) {
              allPartsSufficient = false;
              const partName = goodsMap.get(goods_id) || `ID ${goods_id}`;
              missingPartsMessages.push(`${partName} (kurang ${estimatedQty - issuedQty})`);
            }
          }
      }
      ...
      if (allPartsSufficient) {
        const { error: updateError } = await supabase.from('work_orders').update({ status: 'COMPLETED' }).eq('id', woId);
        if (updateError) throw updateError;
        toast.success("Work Order telah diselesaikan. Semua part sudah diproses.");
        fetchWOs();
      } else {
        toast.error(`Gagal menutup WO. Part belum lengkap: ${missingPartsMessages.join(', ')}`);
      }

    } catch (error: any) {
      console.error("Error finishing WO:", error);
